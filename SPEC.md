...
- 单个测试点编辑或删除。
- 管理后台导出或备份能力。
- 付费顶级域名和多节点高可用。（单机版已支持 DuckDNS 免费子域名、Nginx 与 HTTPS。）
- 深色模式和复杂动画。
- 分布式判题服务。
...
页面风格保持白色背景、黑灰文字、细线边框、克制留白和工具型信息布局。刷题平台不要做营销型首页，第一屏应直接进入可用体验。界面重点是题面阅读、代码提交、表格筛选、状态反馈和错误定位。
...
- 第一版不做深色模式。
- 第一版不做复杂动画。
- 源码编辑器使用 Ace.js，模式固定为 C/C++，并保留加载失败时的清晰错误提示。
- 表单必须有可见 label。
- 错误信息尽量就近显示。
- 按钮、表格、表单、状态标签保持统一样式。
...
- [x] 实现配置读取 MVP。
- [x] 实现日志 MVP：访问日志、错误日志、判题日志。
- [x] 实现统一 JSON 响应结构 `{code, message, data}`。
- [x] 实现静态文件托管。
- [x] 实现健康检查接口。
- [x] 提供一个最小 HTML 首页。
...
- [x] 设计 MySQL schema 文件：`sql/schema.sql`。
- [x] 实现自研固定大小连接池基础设施，后续用于 MySQL 连接句柄。
- [x] 初始化超级管理员账号。
- [x] 实现注册、登录、会话校验、登录过期、登出。
- [x] 实现 RBAC：普通用户、管理员、超级管理员。
- [x] 实现用户提权接口，仅超级管理员可用。
- [x] 用 Google Test 覆盖配置、日志、连接池、会话基础逻辑。

### P3：题库与测试用例管理

- [x] 实现题目创建和编辑接口。
- [x] 实现题目列表接口，支持标题、难度、标签筛选。
- [x] 实现题目详情接口。
- [ ] 实现测试用例 zip 上传。
- [ ] 校验 `.in/.out` 配对、命名、大小、路径穿越。（已完成配对、命名、路径穿越校验；大小限制留到 zip 上传落地时统一接入。）
- [x] 上传成功后整包覆盖该题全部测试点。
- [x] 后台展示测试点列表和上传校验结果。
...
- [x] 构建 C++ Docker 判题镜像。
- [x] 实现编译流程和编译错误截断。
- [x] 实现单测试点运行、超时、内存、输出大小限制。（Docker runner 已限制内存、CPU、进程、输出文件和网络；内存峰值展示仍为 `0`。）
- [x] 实现输出比对和结果分类。
- [x] 实现提交详情接口，返回最终状态和测试点结果。
- [x] 前端提交后每 1 秒轮询，终态后停止。

阶段备注（2026-07-02）：当前 WSL 环境没有 Docker CLI，也没有 MySQL 服务端/客户端开发库，因此 P3/P4 先交付内存仓储 + MySQL schema + 本地 `g++/timeout` runner 的纵向闭环。测试用例上传 HTTP MVP 先提供 JSON 单测试点入口，底层 `TestcaseService` 已支持 `.in/.out` 文件包校验与整包覆盖；真正 zip multipart 解析、Docker 镜像、资源隔离、重启恢复和前端轮询继续保留在后续 P5/P6/P7。

阶段备注（2026-09-01）：公网 MVP 已接入真实 Docker runner，并在 GitHub Actions 的 Ubuntu 主机上通过“注册、建题、上传测试点、提交 C++、Docker 判题为 Accepted”的 API smoke。账号、题目和提交改为单机原子状态文件持久化，满足个人站点重启恢复；MySQL schema 仍保留为后续扩容目标，不应把当前单机状态文件误当成多实例数据库。
...
- [x] 登录页。
- [x] 题目列表页。
- [x] 题目详情与提交页，使用 Ace.js 作为 C++ 编辑器。
- [x] 我的提交页。
- [x] 提交详情页。
- [x] 登出入口。
- [x] 管理员题目管理页。
- [x] 管理员测试用例上传页。（HTTP MVP 仍为 JSON 单测试点入口，zip multipart 后续补。）
- [x] 管理员用户管理页。
- [x] 管理员全站提交页。
- [x] 保持白色克制、工具型风格。
...
- [x] bcrypt 密码哈希。
- [x] session token 只存哈希。
- [x] 注册限流。
- [x] 登录接口基础限流。（当前按 IP 限制登录请求，后续可细分失败用户名 + IP。）
- [x] 提交频率限流。
- [x] 上传大小和文件数量限制。
- [x] Docker 禁止网络访问并限制资源。
- [x] 服务重启后恢复未完成提交为待判题。
- [x] 1 天后清理提交源码和临时目录。（当前实现为启动时清理内存提交源码；落地 MySQL/磁盘源码后需扩展为周期任务。）

### P7：测试与交付

- [x] Google Test 覆盖自研基础设施 MVP。
- [x] 整理常用 `curl` 验证命令。
- [x] 编写 Python `requests` 接口自动化测试。
- [x] 编写 Playwright 主链路测试。
- [x] 写 IP:端口 部署测试说明。
- [x] 提供 DuckDNS 免费域名、Nginx、Let's Encrypt HTTPS 和 systemd 一键部署脚本。
- [x] 每个阶段完成后更新技术日志。
- [x] 每个阶段完成后执行验证、提交 Git，并推送到 GitHub。（本轮 P5/P6/P7 将在提交前完成验证并推送。）
