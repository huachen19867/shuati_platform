...
- [ ] 构建 C++ Docker 判题镜像。
- [x] 实现编译流程和编译错误截断。
- [ ] 实现单测试点运行、超时、内存、输出大小限制。（已完成本地 runner 的运行、超时与输出大小限制；内存限制等待 Docker/cgroup 沙箱接入。）
- [x] 实现输出比对和结果分类。
...
- [x] 提交频率限流。
- [x] 上传大小和文件数量限制。
- [ ] Docker 禁止网络访问并限制资源。（已补 Dockerfile 和 `docker run` 示例；当前 WSL 缺 Docker CLI，真实沙箱执行仍待接入。）
- [x] 服务重启后恢复未完成提交为待判题。
